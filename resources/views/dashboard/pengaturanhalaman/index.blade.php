                ...
                <select class="form-control form-control-sm" name="_halaman_about">
                    ...
                    @foreach ($datahalaman as $item)
                        <option value="{{ $item->id }}"
                            {{ get_meta_velue('_halaman_about') == $item->id ? 'selected' : '' }}> {{ $item->judul }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <select class="form-control form-control-sm" name="_halaman_interest">
                    ...
                    @foreach ($datahalaman as $item)
                        <option value="{{ $item->id }}"
                            {{ get_meta_velue('_halaman_interest') == $item->id ? 'selected' : '' }}> {{ $item->judul }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <select class="form-control form-control-sm" name="_halaman_award">
                    ...
                    @foreach ($datahalaman as $item)
                        <option value="{{ $item->id }}"
                            {{ get_meta_velue('_halaman_award') == $item->id ? 'selected' : '' }}> {{ $item->judul }}
                        </option>
                    @endforeach
                </select>
            </div>
